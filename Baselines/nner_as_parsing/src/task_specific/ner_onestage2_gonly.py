...
class BratNEROneStage2(BratBase):
    def __init__(self, encoder: EncoderBase, **cfg):
        ...
        #print("value of self.cfg.n_span_label: ", self.cfg.n_span_label)
    ...
    def forward(self, x: TensorDict, vp: VarPool) -> TensorDict:
        ...
        if self.training:
            ...
            if self.cfg.train_prune_threshold > 0:
                ...
                with torch.no_grad():
                    ...
                    span_ind[..., 1] += (
                        ...
                        _diff.reshape(-1, L * L).min(1)[0].reshape(-1, 1, 1)
            ...
            if self.cfg.label_pre_marginal:
                ...
                #print("shape of score: ", score["label"].shape)
        else:
    ...
    def loss(
        ...
        if self.training:
            ...
            if self.cfg.label_pre_marginal:
                ...
                #label_loss = self.label_criteria(
                 #   gold_label.view(*x["label"].shape[:2], 1, -1).expand(-1, -1, x["label"].shape[2], -1),
                  #  x["label"]
                #)

                span_head_margin = span_head_margin.view(*x["label"].shape[:2], -1)
                label_loss = (label_loss * span_head_margin).sum()
            #print("shape of gold_label: ",gold_label.shape)
            #print("shape of x[label]: ", x["label"].shape)
