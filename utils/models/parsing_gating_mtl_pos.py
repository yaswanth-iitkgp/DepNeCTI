...
class BiAffine_Parser_Gated(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, use_pos, use_char, pos_dim, num_pos, num_filters,
        ...
        self.sequence_tagger_decoder_morph = Tagger_Decoder(hidden_size, arc_tag_space, 283, p_out, initializer)
        self.sequence_tagger_decoder_case= Tagger_Decoder(hidden_size, arc_tag_space, 16, p_out, initializer)
        self.sequence_tagger_decoder_label = Tagger_Decoder(hidden_size, arc_tag_space, 28, p_out, initializer)
    ...
    def loss_morph(self, input_word, input_char, input_pos, mask=None, length=None, hx=None):
        encoder_output, hn, mask, length = self.rnn_encoder(input_word, input_char, input_pos, mask, length, hx)
        out_counter = self.sequence_tagger_decoder_morph(encoder_output, mask)
        loss_ = self.sequence_tagger_decoder_morph.loss(out_counter, input_pos, mask, length)
        return loss_
    def loss_case(self, input_word, input_char, input_pos, mask=None, length=None, hx=None):
        encoder_output, hn, mask, length = self.rnn_encoder(input_word, input_char, input_pos, mask, length, hx)
        out_counter = self.sequence_tagger_decoder_case(encoder_output, mask)
        loss_ = self.sequence_tagger_decoder_case.loss(out_counter, input_pos, mask, length)
        return loss_
    def loss_label(self, input_word, input_char, input_pos, mask=None, length=None, hx=None):
        encoder_output, hn, mask, length = self.rnn_encoder(input_word, input_char, input_pos, mask, length, hx)
        out_counter = self.sequence_tagger_decoder_label(encoder_output, mask)
        loss_ = self.sequence_tagger_decoder_label.loss(out_counter, input_pos, mask, length)
        return loss_
    ...
    def pre_loss(self, out_arc, out_arc_tag, heads=None, arc_tags=None, mask=None, length=None, use_log=True, temperature=1.0):
        ...
        if not decode:
            ...
            #with open('../new_file1.txt','a') as f:
                ...
                #f.write("arc_tag_space = " + str(arc_tag_space)+'\n')
                #f.write("heads = " + str(heads.shape)+'\n')
                #f.write("arc_tag_h = " + str(arc_tag_h.shape)+'\n')
                #f.write("batch_index:" + str(batch_index)+'\n')
                #f.write("heads.data.t(): " + str(heads.data.t())+'\n')
                #f.write("arc_tag_h: " + str(arc_tag_h)+'\n')
            # get vector for heads [batch_size, length, arc_tag_space],
